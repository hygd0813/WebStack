<?php if (!defined('__TYPECHO_ROOT_DIR__')) exit; ?>
 <div class="part">
    ...
    <div id="<?php $this->respondId(); ?>" class="respond">
    	...
    	<h3 id="response" class="tt posttop"><?php _e('添加新评论'); ?></h3>  
    	<form method="post" class="items" action="<?php $this->commentUrl() ?>" id="comment-form" role="form">
            ...
            <?php endif; ?>
    		<div>
                <textarea rows="8" cols="50" class="text" name="text" id="txaArticle" placeholder="<?php _e('说点什么吧'); ?>" class="textarea" required><?php $this->remember('text'); ?></textarea>
            </div> 
            
    		<p><button type="submit" class="submit2 button transition"><?php _e('提交评论'); ?></button></p>
    	</form>
    </div>
    </div>
     ...
     <?php $this->comments()->to($comments); ?>  
            <ul class="msg msghead">
                <li class="tbname">
                    <p class="tt">
                        <span>评论列表
                            （<span class="emphasize"><?php $this->commentsNum(_t('0'), _t('1'), _t('%d ')); ?></span>条） 
                        </span>
                    </p>
                </li>
            </ul>
            <div class="items">
                <i class="fa fa-smile-o"></i>&nbsp;<?php $this->commentsNum(_t('暂无评论'), _t('仅有一条评论'), _t('已有 %d 条评论')); ?><label id="AjaxCommentBegin"></label>
                <!--评论输出-->
